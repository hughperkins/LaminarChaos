...
#define NTHROW(m) cout <<  m << endl;

namespace LaminarChaos {

MeshModel :: MeshModel()
{
	 cout <<  "Model::MeshModel() constructor" << endl;
	// clear everything
	numframes=0;
	fps=0.0f;
	frames=NULL;
...
bool MeshModel :: Setup(int num,float framesec)
{
	// allocate the frames
	try {
...
void MeshModel :: Add(int num,ModelFrame &frame)
{
...
bool MeshModel :: Load(std :: string name,ModelLoaderClass &loader )
{
	// attempt to load the file
	if(!loader.Load(name))
	{
	   cout << "Error, OurRef:LaminarChaos::MeshModel::Load, Unable to load file " << name << endl;
	   return false;
	}
	...
	BoundingBoxMin = loader.GetBoundingBoxMin();
	BoundingBoxMax = loader.GetBoundingBoxMax();

	// tidy up the values
	if(modelframes<=0)
	{
	   cout << "Error, OurRef:LaminarChaos::MeshModel::Load, file " << name << " has no frames" << endl;
	   return false;
	}
	...
	{
	   cout << "Error, OurRef:LaminarChaos::MeshModel::Load, couldnt set up modelframes for file " << name << endl;
	   return false;
	}
	...
	for(int ct=0; ct<numframes; ct++)
	{
		...
		{
	          cout << "Error, OurRef:LaminarChaos::MeshModel::Load, couldnt set up meshes for file " << name << endl;
			this->Clear();
			return false;
		}
		...
		for(int ct2=0; ct2<nummeshes; ct2++)
		{
			...
			// get the mesh
			if(!loader.Get(ct,ct2,&msh))
			{
	               cout << "Error, OurRef:LaminarChaos::MeshModel::Load, couldnt get mesh " << name << endl;
				//cout <<  "Model::load() problem loading mesh" << endl;
				return false;
			}
			else
			{
				//cout <<  "model::load mesh.tex=" << msh.GetTexture() << endl;
			}
...
void MeshModel :: DrawFrame(int frame)
{
...
void MeshModel :: DrawTimeFrame(int startframe,float time)
{
	...
	//cout <<  " model framefirst text " << frames[first].meshes[0].tex << " inter " << inter.meshes[0].tex << endl;
	
	// cout <<  "DrawTimeFrame " << inter.meshes[0].tex << endl;
