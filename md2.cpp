...
bool MD2 :: Load(std :: string file)
{
	...
	{
		 cout <<  "couldnt read md2 header from file " << file << endl;
		fclose(fp);
		return false;
	}
  ...
  bool bInitializedMaxMinBoundingbox = false;
	...
	for(ct=0; ct<numframes; ct++)
	{
		...
		for(int ct2=0; ct2<numverts; ct2++)
		{
			...
			/*
			verts[ct][ct2].x=((float)tempverts[ct2].v[0]);
			verts[ct][ct2].y=((float)tempverts[ct2].v[1]);
			verts[ct][ct2].z=((float)tempverts[ct2].v[2]);
			*/
			
			float thisx, thisy, thisz;
			thisx = verts[ct][ct2].x;
			thisy = verts[ct][ct2].y;
			thisz = verts[ct][ct2].z;
			
			if( !bInitializedMaxMinBoundingbox )
			{
				cout <<  "md2: initializing boundingbox" << endl;
				bInitializedMaxMinBoundingbox = true;
				boundingboxmin.x = boundingboxmax.x = thisx;
				boundingboxmin.y = boundingboxmax.y = thisy;
				boundingboxmin.z = boundingboxmax.z = thisz;
			}
			
			if( thisx < boundingboxmin.x )
			{
				 boundingboxmin.x = thisx;
		  }
			if( thisy < boundingboxmin.y )
			{
				 boundingboxmin.y = thisy;
		  }
			if( thisz < boundingboxmin.z )
			{
				 boundingboxmin.z = thisz;
		  }

			if( thisx > boundingboxmax.x )
			{
				 boundingboxmax.x = thisx;
		  }
			if( thisy > boundingboxmax.y )
			{
				 boundingboxmax.y = thisy;
		  }
			if( thisz > boundingboxmax.z )
			{
				 boundingboxmax.z = thisz;
		  }
		}
//		cout <<  "boundingbox for this frame: " << boundingboxmin << " " << boundingboxmax << endl;
	}
	...
	for(ct=0; ct<head.numskins; ct++)
	{
		...
		cout <<  "texture/skin name: " << skinname << endl;
	}
...
bool MD2 :: Get(int frameno,int meshno,Mesh *mesh)
{
